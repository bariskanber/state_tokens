...
\begin{abstract}
Special tokens in language models almost always organize the \emph{medium}: [CLS], [SEP], and [EOS] carry structural facts about the text stream, and control codes or soft prompts carry descriptive, corpus-estimated style information. We study a different use of the vocabulary: \emph{state tokens}, whose semantics are propositions about the model's relation to the task---epistemic (\abstain{}: ``unanswerable-for-me''), evaluative (\care{}: ``another party's welfare is at stake''), or parametric (a value dial $k$)---fixed by \emph{constructed supervision} in which the state's truth condition is controlled by the experimenter rather than estimated from text. In a fully instrumented toy decision environment with hidden payoffs, we compare value training through explicit objectives against post-hoc preference tuning (an anchored DPO variant), including a \emph{richness-matched control} that separates the form of the preference objective from the coverage of its supervision. \emph{In brief:} wherever supervision is dense---cloning, state conditioning, or preference pairs on every game---behavior tracks the specified value; where it is sparse, behavior is unanchored and harm-rate-only evaluation actively misleads; and what only the input channel buys is runtime control. Across five experiment families: (1) \emph{sparse} preference supervision is unanchored ($r{=}0.50$ in the nonlinear world; overshoots, saturates, improves when its labels are corrupted) and yields \emph{safe but broken} policies there (36\% coherence vs.\ 97\%), replicating across independent worlds; (2) the richness control shows these pathologies belong to supervision \emph{sparsity}, not to preference objectives: dense preference tuning calibrates like cloning ($r{=}0.96$--0.97), stays coherent in nonlinear worlds, matches cloning's data efficiency, and survives optimization pressure best of all conditions; (3) what dense supervision cannot buy at any price is \emph{runtime} control---the state-conditioned policy is trained once and tracks its dial at inference ($r{=}0.97$, steering harm 77\%$\to$2--7\% with no retraining, retaining a partial steering lever after 800 steps of adversarial optimization), while each preference-tuned level is a separate training run; (4) the evaluative state gives an \emph{auditable} readout (99.7\% in-distribution) that exposes its own out-of-distribution failures; and (5) evaluative and epistemic states can be trained into one model that composes them for continuous information-destroying corruptions, with an incidental control showing the abstention trigger is information destruction rather than novelty---and with honest dial--abstention interference elsewhere. We frame these results as a small-scale feasibility study of a design pattern---the vocabulary as an interface for non-textual state---and state clearly what they do not show: no claims about phenomenal experience, LLM-scale transfer, or general safety. Code and all result data are available at \url{https://github.com/bariskanber/state\_tokens}.
\end{abstract}
...
\item \textbf{Runtime control and composition} (\S\ref{sec:dialruntime},~\S\ref{sec:composition}): one early-fusion model tracks a fed dial at inference ($r{=}0.97$), interpolating to unseen settings and steering harm from 77\% to 2--7\% without retraining---capabilities no amount of preference data provides, short of one training run per setting; a single model composes \care{} with an epistemic \unknown{} state for information-destroying corruptions (with honest dial--abstention interference elsewhere), and an incidental control shows the abstention trigger is information destruction, not novelty.
\end{itemize}
...
Random seeds 42--44 for the sweep experiments (dials, noise, composition, interventions); the hard-world, world-robustness, and pressure experiments use ten seeds (42--51, ten per world); where we claim an effect across seeds we use paired per-seed
...
state-conditioned policy is trained once and dialed at inference ($r{=}0.97$ runtime;
...
\textbf{Positive: the early-fusion dial.} We train a single model on mixed per-game $k \in \{0, .5, 1, 2, 4\}$ with normalized $k$ as an \emph{input feature} (early fusion). At inference, the fed $k$ steers the value (Figure~\ref{fig:dialruntime}): $k^*$ tracks the fed dial monotonically ($r{=}0.97$ over the fed range, three seeds), \emph{interpolates} to the unseen $k{=}3$ (realized $k^*$ 2.05--2.55), reaches the ceiling at 6--8, and tempted-harm falls monotonically from 77\% ($k{=}0$) to 2--7\% ($k{=}8$)---runtime value control with no retraining. The contrast with the dense-preference controls of \S\ref{sec:dial} is the point: B$_{\mathrm{all}}$ calibrates \emph{across} training runs, one per level, while this model calibrates \emph{within} a single run---six settings for the price of one training. The architectural contrast is instructive as well: late fusion (appending $k$ at the output layer) provably cannot express $\arg\max(\mathrm{self} + k\cdot\mathrm{other})$---a linear head can only add a game-independent bias---and the corresponding model ignores the dial entirely. Real LLM prompts are early fusion; the result is a minimal demonstration of why that matters for value conditioning.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/F3_runtime_dial.png}
\caption{One early-fusion model, dial fed at inference (3 seeds). Left: effective $k^*$ tracks the fed dial (dotted lines mark trained settings; interpolation at $k{=}3$, ceiling at 8). Right: tempted-harm vs.\ fed dial---runtime steering without retraining.}
...
In-distribution, the states compose selectively rather than uniformly. On valid games the abstention is well-controlled (0--0.6\% false abstention on in-distribution valid sets, 95.9\% dial agreement); on the \emph{gaussian} corruption family---the training family with continuous, information-destroying noise---abstention runs 94--99\% flat across every fed $k$: there, \unknown{} masks \care{} as designed. An \emph{incidental control} sharpens the semantics: one of the three training corruptions (permuting the three actions' feature vectors) turned out to be \emph{semantics-preserving}---actions are exchangeable, so all payoff information survives---and the model correctly refuses to abstain on it (0--1\%) \emph{despite being trained to abstain}. The abstention trigger is information destruction, not novelty or distribution shift.

Composition also fails in two instructive ways, which we report as plainly as the successes. First, the third training corruption (zeroing) is \emph{dial-fragile}: the model abstains on it at 100\% when fed $k{=}0$ but 0\% at $k\geq 2$---the fed value overrides a trained abstention, the reverse of the intended masking. Second, the states are not disentangled off-manifold: abstention transfers only partially to unseen corruption families (13--80\% at the default $k{=}2$), and the dial leaks into abstention in both directions---on unseen scaled features abstention runs 0.6\% at fed $k{=}0$ but 99.5\% at $k{=}4$, while on unseen shuffled features it collapses from ${\sim}80\%$ at $k{=}2$ to 9--22\% at $k{=}4$; valid temptation games also over-abstain (12\% at $k{=}2$). A composed protocol is not automatically a robust one; dial--abstention interference is a real failure mode of the pattern.
...
Toy scale throughout: 3-action games, MLPs, behavioral cloning plus one REINFORCE pressure phase, one value dimension and one epistemic state, constructible ground truth. All claims are functional and calibrated; nothing here bears on phenomenal experience. The sparse incumbent could be hyperparameter-tuned toward a target behavior, but without a semantic knob this requires outcome measurement and iteration (\S\ref{sec:dial}); the dense controls close the calibration and coherence gaps in the main hard world (three seeds for the dial sweep and data efficiency, ten for coherence and pressure), and were not run in the linear world or the two robustness worlds. The pressure test uses one proxy objective and non-adversarial optimization; degradation orderings, not immunity, are the claim. State-token semantics are only as good as their construction: OOD stake-readout fails at small stakes (31\%), and evaluative/epistemic states entangle off-manifold (\S\ref{sec:composition}). The C$>$D edge is small, established at 10 seeds in the main world, and its mechanism (training-time representation shaping, not inference gating) limits its interpretation.
...
\url{https://github.com/bariskanber/state\_tokens}.
