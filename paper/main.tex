...
\textbf{Lesson.} In an LM, a state token lives in the input stream, so a report head attached to it can shortcut to echoing the token. Auditability by construction requires not just a truth condition but \emph{channel discipline}: the channel that reports a state must not be the channel that carries it. We scope the bridge plainly: DistilBERT, three seeds, one task rendering---mechanism demonstrated (steering, circularity, repair), not LLM performance claimed.
...
auditable (97.4\%) with zero behavioral change---and since state labels are free by
