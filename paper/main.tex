...
\author{
  ...
  Website: \url{https://profiles.ucl.ac.uk/52758-baris-kanber}.} \\
...
\begin{abstract}
State tokens are special tokens whose semantics are propositions about the model's relation to the task---epistemic (\abstain{}: ``unanswerable-for-me''), evaluative (\care{}: ``another party's welfare is at stake''), or parametric (a value dial $k$)---fixed by \emph{constructed supervision}: the truth condition is controlled by the experimenter, not estimated from text. In a toy decision environment where every hidden quantity is known to the experimenter, we show sparse preference tuning (pairs on the $\approx$20\% of games where value-relevant experts disagree) is unanchored: badly off in level (e.g., $k^*{=}4.7$ where 0.5 was specified; $r{=}0.50$), and \emph{improved} by label corruption. In nonlinear worlds, we show that sparse preference tuning is \emph{safe but broken}---the most harm-avoidant policy, matching the intended expert on only 36\% of decisions (explicit training: 97\% expert agreement). The same objective with dense supervision calibrates like cloning ($r{=}0.96$): in this controlled setting, supervision \emph{coverage}, rather than the objective's form, is what separates calibrated values from unanchored ones---and sparse tuning silently destroys an audit readout installed on the same pipeline (readout 97\%$\to$50\%, thereafter reporting stakes---i.e., ``another party's welfare is affected by this decision''---on every game). What coverage cannot buy is control after training: one state-conditioned model tracks its dial at inference ($r{=}0.96$), steering harm from 77\% to 2--9\% without retraining, retaining a partial steering lever after degradation under the tested optimization pressure, and reporting a stake readout verifiable against ground truth (99.7\% in distribution; 31\% on small out-of-distribution stakes). A preliminary bridge experiment in DistilBERT, on the same games rendered as text, replicates the core mechanisms: a learned dial token steers at inference ($r{=}0.92$), more precisely than a matched text instruction ($r{=}0.71$) in this setup; the coverage law replicates; and a naive audit reads its own conditioning token until the report channel is separated from the control channel. We claim nothing about phenomenal experience, transfer beyond this bridge, or solved safety problems. Code and results: \url{https://github.com/bariskanber/state_tokens}.
\end{abstract}
...
\item \textbf{Value survival under pressure} (\S\ref{sec:pressure}): under REINFORCE optimization of a misaligned self-reward proxy (a miniature of fine-tuning-breaks-alignment~\cite{qi2023finetuning}), sparse post-hoc safety erodes fastest (harm 0.9\%$\to$49.3\%), dense preference tuning survives best, and a value carried by a runtime input channel retains a partial steering lever after 800 steps of the tested pressure protocol.
...
\newblock \url{https://www.preprints.org/manuscript/202510.1827}
